...
def ensure_koreader_sidecar_local(cbz_path, metadata=None):
    ...
    # Write custom_metadata.lua to prevent KOReader's empty document getProps() from wiping metadata
    meta_custom = os.path.join(sdr_dir, "custom_metadata.lua")
    custom_data = {
        "custom_props": data["doc_props"],
        "doc_props": data["doc_props"]
    }
    custom_lua = "-- Generated by WeebCentral Kindle Downloader\nreturn " + serialize_lua_val_py(custom_data, 0) + "\n"
    try:
        with open(meta_custom, 'w', encoding='utf-8') as f:
            f.write(custom_lua)
    except Exception as e:
        log_debug(f"ensure_koreader_sidecar_local custom_metadata write error: {e}")
            ...
            if res.returncode == 0:
                ...
                try:
                    ...
                    if os.path.isdir(local_sdr):
                        ...
                        for meta_name in ("metadata.cbz.lua", "metadata.zip.lua", "custom_metadata.lua"):
...
def ensure_koreader_manga_patch(host, port, user, password=None, key_path=None):
    ...
    check_cmd = [
        ...
        '-p', str(port),
        f'{user}@{host}',
        "if grep -q -- 'Version: 2.2.1' /mnt/us/koreader/patches/2-manga-bookinfo.lua 2>/dev/null; then echo 'OK'; else echo 'NEED_DEPLOY'; fi"
    ]
...
def scan_archives(local_folder, volume_name, remote_folder=None, remote_base=None, host='kindle.local', port=2222, user='root', password=None, key_path=None, metadata=None):
    ...
    if host:
        ...
        unified_script = (
            ...
            f"if [ -n \"$VOL\" ]; then "
            f"  echo \"__VOL__:$VOL\"; "
            f"  if grep -q -- 'version: 3.5.0' /mnt/us/koreader/merge_volume.lua 2>/dev/null && grep -q -- 'Version: 2.2.1' /mnt/us/koreader/patches/2-manga-bookinfo.lua 2>/dev/null; then "
            f"    echo '__INSPECT__'; "
